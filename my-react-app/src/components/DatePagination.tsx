...
import { useEnergyDashboardContext } from '../AppProvider';
...
const DatePagination: React.FC<DatePaginationProps> = ({ selectedDate, onDateChange }) => {
  ...
  const calculatePickerPosition = () => {
    if (datePickerButtonRef.current) {
      ...
      // Calculate ideal position (below button, right-aligned)
      ...
      if (top + pickerHeight > windowHeight + scrollTop - 10) {
  ...
  const handleDatePickerToggle = () => {
    if (!showDatePicker) {
      // Calculate position after a brief delay to ensure state update
      setTimeout(() => {
  ...
  // Effect to recalculate position when picker becomes visible
  useEffect(() => {
    if (showDatePicker) {
  ...
  return (
    <div 
      className={`${styles.datePagination} ${theme === 'dark' ? styles.darkTheme : styles.lightTheme}`}
      role="group"
    ...
    >
      ...
      <div className={styles.daysContainer}>
        {weekDates.map((date, index) => (
          <button
            ...
            // Remove inline styles to let CSS classes handle all styling
            >
      ...
      {showDatePicker && createPortal(
        <div className={`${styles.portalOverlay} ${theme === 'dark' ? styles.darkTheme : styles.lightTheme}`}>
          <div 
            className={styles.portalBackdrop}
            onClick={() => setShowDatePicker(false)}
          />
          <div 
            className={`${styles.portalDatePicker} ${theme === 'dark' ? styles.darkPortalPicker : styles.lightPortalPicker}`}
            style={{
              position: 'fixed',
